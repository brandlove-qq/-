...
#define  filepath      "E:/newworkcode/sssp/fattree768nodes.topo"
#define  filepath1     "E:/newworkcode/sssp/test_tuopu.txt"
#define  outputfile_rule_test "E:/newworkcode/sssp/topo_output.txt"
#define  outputfile_write_infile "E:/newworkcode/sssp/result/"
...
//跨板多端口限制记录-消息记录结构体--一个入端口对应多个端口的限制

typedef  struct  across_board_handle
{
    string   current_switch_name;  //当前交换机名

    string  target;             //到达的目标节点

    string   across_switch_name;  //跨越的交换机 ru:144

    //string  across_switch_first_conect_switch_name; //跨越后第一个交换机，如9：144-9
...
typedef struct Node
{
    ...
    //对象数组（管理负载平衡）--每个对象负责自己的接口轮询--
    std::vector<LoadBalancer> p1;          //
    //     LoadBalancer  *loadBalancers;   // 用智能指针管理资源
    int  loadbalanceobject_size;     //对象个数

    // 对板内多链路负载平衡:多个端口经行限制
    bool  is_limitport_swit;              // 链接了终端节点和没有多链接端口的交换机不需记录出端口数据 ---hnr3
    string  inport_targetname;            //入端口-目的
    std::unordered_map  <std::string, mutilport_handle>   getnextserver_record; //已分配的出端口链路

    //跨板一对多端口限制--类似板内多端口
    std::unordered_map <std::string , across_handle_limit>  across_record;       //hnr3 记录信息
...
typedef std::function<void(std::vector<string> &,\
                                 std::unordered_map<std::string,switch_node> &,\
                                          const int)>  callback;
